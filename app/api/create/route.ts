...
export async function POST(request: Request) {
  ...
  try {
    ...
    body.forEach(async (doc: any) => {
      const dataEntitas = doc.entitas.map((doc: any) => ({
        id: doc.id,
        ...doc,
      }));
      ...
      await prisma.header.create({
        data: {
          ...
          tanggal_daftar: moment(doc.tanggal_daftar).format("yyyy-MM-dd'T'HH:mm:ssZ"),
      ...
      // try {
      //   await prisma.header.create({ data: result })
  ...
  //   const dataHeader = body.map((doc:any) => (
  //     {
  //           id: doc.id,
  //           nomor_aju: doc.nomor_aju,
  //           kode_dokumen: doc.kode_dokumen,
  ...
  // }));
  // // console.log(body[0].entitas)
  //   const dataEntitas = body[0].entitas.map((doc:any) => ({
  //     id: doc.id,
  //     ...doc,
  // }));
  ...
  // console.log(data)
  // try {
  //     await prisma.header.create({ data:data})
  //     return Response.json('message: success',{
  //       status: 200,
  //     })
  //   } catch (error) {
