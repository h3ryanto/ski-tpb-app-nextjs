...
export function ImportDataExcell({ saveUrl, reload }: { saveUrl: string, reload: () => Promise<void> }) {
    ...
    return (
        <Dialog modal={false}>
            ...
            <DialogContent className="max-w-full h-full flex items-center justify-center bg-black bg-opacity-40 z-[7777]"
            ...
            >
                <div className="w-[80vw] bg-slate-50 p-7 top-10">
                    ...
                    <DialogFooter>
                        ...
                        <DialogTrigger asChild>
                            <Button onClick={() => { setData([]); setFile(null); setSuccess(false); setTab([]); }}>Close</Button>
                        </DialogTrigger>
                    </DialogFooter>
                </div>
            </DialogContent >
