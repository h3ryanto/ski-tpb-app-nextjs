...
import { FileTextIcon, InboxIcon, RefreshCcwIcon } from "lucide-react";
...
const Archive = (props: { searchParams: SearchParams }) => {
  ...
  const loadData = async (search: any, limit: number, currenPage: number) => {
    ...
    const data = await fetch(
      `/api/get-archive?page=${currenPage}&size=${limit}&search=${search}`,
      {
        method: "GET",
        headers: {
            ...
            <table className="table-auto hidden md:table w-full">
              <thead className="top-10 ">
                <tr className="border-b-2 border-y-slate-400 sticky -top-1 bg-slate-100">
                  ...
                  <th scope="col" className="align-top p-2">
                    <div className="pt-2">Nomor Dokumen</div>
                  </th>
                ...
                {(data.length > 0 &&
                  data.map((post: any, index: number) => (
                    <tr key={index} className=" hover:bg-slate-100/65">
                      ...
                      <td className="p-2">
                        {format(new Date(post.tanggal_dokumen), "dd-MM-yyyy")}
                      </td>
